...
export const stepsSetup: Step[] = [
  {
    ...
    help: 'Enter the current track levels (1–7) for Russia and the US across all 7 Strategic Capabilities, plus any Sanctions markers.',
    inputs: [
      ...CAPABILITY_KEYS.map((k) => ({
        id: `faction_${k}`,
        kind: 'int' as const,
        label: `Russia — ${CAPABILITY_LABELS[k]}`,
        min: 1,
        max: 7,
      })),
      ...CAPABILITY_KEYS.map((k) => ({
      ...
      ...CAPABILITY_KEYS.map((k) => ({
        id: `sanctions_${k}`,
        kind: 'bool' as const,
        label: `Sanctions marker on Russia ${CAPABILITY_LABELS[k]}?`,
      })),
    ],
    resolution: {
      kind: 'custom',
