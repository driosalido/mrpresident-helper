...
export const stepsSetup: Step[] = [
  {
    ...
    help: 'Enter Russia and US capability track levels (1–7) and the current Russia–US Relation level and trend.',
    inputs: [
      ...
      })),
      {
        id: 'usRelationLevel',
        kind: 'enum' as const,
        label: 'Russia–US Relation Level',
    ...
    resolution: {
      ...
      resolve: (ctx) => {
        ...
        const soe = Number(ctx.inputs['soe'] ?? 4);
