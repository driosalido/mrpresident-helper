...
export const stepsC: Step[] = [
  {
    ...
    help: (ss) => {
      ...
      return `${attempts} attempts (Posture ${posture}). Priority: (1) lag ≥ 2 vs US → (2) Cyber + Naval Forces → (3) d10 table. Improvement success ≤ ${threshold}.`;
    },
    inputs: [
      {
        id: 'sanctionsCount',
        ...
        for (let i = 0; i < attempts; i++) {
          ...
          if (modified <= threshold) {
            ...
            outcomes.push({
              ...
              summary: `Attempt ${i + 1}: **${label}**${selNote} — roll ${impRoll}${sanctionsNote} = ${modified} ≤ ${threshold} — SUCCESS. China ${label} → ${tracks.faction[target]}.`,
            });
          } else {
            outcomes.push({
